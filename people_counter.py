...
def send_mail() -> None:
    """Send email alert when people limit is exceeded.
    
    Reads recipient email from config and sends alert notification.
    """
...
def log_data(
    ...
    Args:
        move_in: List of movement-in counts
        in_time: List of entry timestamps
        move_out: List of movement-out counts
...
def people_counter() -> None:
    ...
    This function:
    - Loads MobileNetSSD model for person detection
    - Initializes video stream (webcam, RTSP, or video file)
    - Tracks people using centroid tracking
    - Counts people entering and exiting
    - Sends alerts when threshold is exceeded
    - Logs data to CSV file
    
    Configuration is read from utils/config.json.
    """
